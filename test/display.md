...
来源：`test/test.md` 的 58 个测试标题。每个小节都标出对应英文 fixedName 和入口 JS 文件，并保留当前插件状态下用于观察效果的演示内容。
...
尝试修改这一句、勾选下面任务项或粘贴文本，应被阻止或明显受限：- [ ] 只读模式测试项。
...
![无图模式应被隐藏或替换占位](./assets/sample.svg)

图片应隐藏或显示占位信息，正文排版不应被图片撑开。
...
下面每一项对应的语法边界都应更稳定地显示出来，方便逐项核对配置面板里的勾选项。

- 粗体：**粗体的 `**` 应常显**
- 斜体：*斜体的 `*` 应常显*
- 删除线：~~删除线的 `~~` 应常显~~
- 下划线：<u>下划线的 `<u>` 和 `</u>` 应常显</u>
- 上标：version^2026^
- 下标：CO~2~
- 代码：`反引号应常显`
- 图片：![图片标记应常显](./assets/sample.svg)
- 链接：[链接括号和地址应常显](https://github.com/obgnail/typora_plugin)
- 脚注：脚注引用标记应常显[^static-markers-enabled-footnote]
- 高亮：==高亮的 `==` 应常显==
- 表情：:heart:
- 行内公式：$\sum_{i=1}^{n} i = \frac{n(n+1)}{2}$
- 行内 HTML：<span data-static-marker="enabled-inline-html">行内 HTML 标签应常显</span>

[^static-markers-enabled-footnote]: 效果里的脚注定义。
...
当前阅读附近保持清楚，远离焦点的行应逐渐弱化，移动光标可以观察变化。
...
![图片放缩样例](./assets/sample.svg)

选中或拖动图片，应能调整显示尺寸，并保留新的宽高信息。
...
右键表格选择“增强表格”，应能看到搜索框、每页条数、列排序、列筛选下拉框和分页信息；可以搜索 `diagram`、按“状态”排序，或在“类型”列筛选 `chart`。
...
![图片查看样例 A](./assets/sample.svg)

![图片查看样例 B](./assets/PixPin_2026-06-05_21-08-06.webp)
...
这是一段很长很长的文字，用来观察文段截断效果。超出配置长度的部分应被折叠或隐藏，并提供展开全部的操作入口。继续补充一些文字，让这一段足够长：alpha beta gamma delta epsilon zeta eta theta iota kappa lambda mu。
...
在左侧文件树观察 `test/`：应出现文件计数、扩展信息或增强操作。本文档用于制造可见文件：`display.md`。
...
对本段或当前文件执行加密操作，应生成不可直接阅读的密文；解密后应恢复：API_TOKEN = display-secret-token-123。
...
在这段区域按配置手势移动，应出现轨迹或触发返回、关闭、滚动等动作。
...
外部客户端应能通过配置的 RPC 接口读取路径、内容或执行允许的操作；未授权请求应被拒绝。
